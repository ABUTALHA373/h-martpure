@props([
    ...
    'optionValue' => 'value',
    'optionLabel' => 'label',
    'dataKeys' => [] // Array of keys to bind as data attributes
])
...
<div x-data="{
    ...
    <button @click="open = !open" @click.outside="open = false" type="button"
            class="w-full h-9 px-3 text-left rounded-md border border-custom bg-bg-primary text-text-primary
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-100"
         ...
         class="absolute z-50 w-full mt-1 rounded-lg border border-custom bg-bg-primary shadow-lg max-h-60 overflow-auto py-1"
         style="display: none;">
        ...
        <template x-for="option in options" :key="option['{{ $optionValue }}']">
            <div @click="select(option['{{ $optionValue }}'])"
                 ...
                 :class="{
                     'bg-secondary/10 text-secondary': selected == option['{{ $optionValue }}'],
                     'text-text-primary hover:bg-bg-secondary': selected != option['{{ $optionValue }}']
                 }"
                 x-bind="{
