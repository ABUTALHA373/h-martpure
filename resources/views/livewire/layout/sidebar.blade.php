<div class="p-2 py-4 flex flex-col gap-1 relative">
    @php
        $nav_cl_a = "flex items-center gap-3 px-2 py-2 rounded-lg bg-secondary/10 text-secondary ";
        $nav_cl = "flex items-center gap-3 px-2 py-2 rounded-lg text-text-secondary hover:bg-bg-secondary hover:text-text-primary ";
    @endphp
    <a wire:navigate
    ...
    @adminHasPermission('settings.view')
    <a wire:navigate
       href="{{ route('admin.siteSetting') }}"
       class="{{ request()->routeIs('admin.siteSetting') ? $nav_cl_a : $nav_cl}}">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5 text-secondary">
            <path fill-rule="evenodd"
